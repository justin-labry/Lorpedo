...
public class Main implements Runnable {
	...
	public Main(int port, int count) throws IOException { //Constructor 
		...
		MacLearningManager macLearningManager = new MacLearningManager();
		//PacketDumpManager packetDumpManager = new PacketDumpManager();
		LinkDiscoveryManager linkDiscoveryManager = new LinkDiscoveryManager();
		
		handlers.add(macLearningManager);
		//handlers.add(packetDumpManager);
		handlers.add(linkDiscoveryManager);
	}
	...
	public static void main(String[] args) throws IOException {
		...
		if(args.length < 1) {
			System.out.println("usage ./Lorpedo [number of threads]");
			System.exit(0);
		} else {
			count = Integer.parseInt(args[0]);
		}

		Main server = new Main(6633, count);

		server.start();
		
		System.out.println("Lorpedo is activated with " + count + " threads..");

		//	check if there is no connection then call server stop
		//		server.stop();
